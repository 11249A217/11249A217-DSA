...
To perform Breadth First Search (BFS) traversal (also called Level Order Traversal) of a binary tree in C using a queue.
...
If the root node is NULL, stop.

Insert (enqueue) the root node into the queue.

While the queue is not empty:

Remove (dequeue) a node from the front of the queue and store it as temp.

Visit (print) temp->data.

If temp->left is not NULL, enqueue temp->left.

If temp->right is not NULL, enqueue temp->right.
